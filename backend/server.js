...
app.post('/api/prompts/generate', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Check cache first
    const cacheKey = `prompt:${genres.sort().join(':')}`;
    ...
    if (cachedPrompt) {
      ...
      // Still log the generation for analytics
      await logPromptGeneration(userId, genres, prompt);
      
      promptGenerationCounter.add(1, { source: 'cache' });
      span.setStatus({ code: 1 });
    ...
    // Cache the result
    await redis.setex(cacheKey, 3600, JSON.stringify(prompt)); // Cache for 1 hour
