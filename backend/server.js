...
app.post('/api/drawing/feedback', async (req, res) => {
  const span = tracer.startSpan('drawing-feedback-generate');
  ...
  try {
    ...
    // Call prompt service
    const promptServiceResponse = await axios.post(
      'http://prompt-service:5001/generate-drawing-feedback',
      {
        image,
        exercise,
        skills,
    ...
    console.error('Drawing feedback generation error:', error);
    res.status(500).json({ error: 'Failed to generate drawing feedback' });
  } finally {
    span.end();
  }
});
